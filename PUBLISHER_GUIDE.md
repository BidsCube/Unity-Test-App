...
- **`bidscube`**: `baseUrl`, `appKey`, `publisherId`, placement IDs (banner / video / native).
...
The **Direct SDK** panel reads placements and base URL via **`BidscubeDemoRuntimeConfig`**. The **AppLovin** and **LevelPlay** panels still allow typing in the UI; PlayerPrefs override JSON for local iteration.

---

## How to run the Direct SDK demo

1. `./tools/use-demo-profile.sh direct`
2. Open **`Assets/Sample scene.unity`**, press **Play**.
3. Choose **1 · Direct SDK**, then **Initialize SDK**, then **Banner** / **Video** / **Native**.

If **`baseURL`** on the scene’s `TestIntegration` is empty, the demo uses **`bidscube.baseUrl`** from JSON when present.
