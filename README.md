...
**UPM (релізні теги на GitHub):** у **`Packages/manifest*.json`** пакети **`com.bidscube.*`** підключені лише через **Git URL + тег** — **v1.2.10** (core), **v1.0.20** (AppLovin MAX), **v1.0.5** (LevelPlay). Локальні **`file:../../…`** у цьому репозиторії не використовуються (перевіряє **`tools/verify-demo-profiles.sh`**).
...
**Очікувані версії пакетів (UPM):** core SDK **v1.2.10**, AppLovin adapter **v1.0.20**, LevelPlay adapter **v1.0.5** (split Android AARs `bidscube-sdk-lite-no-video-1.2.4.aar` / `bidscube-sdk-full-video-1.2.4.aar` у пакетах адаптерів).
